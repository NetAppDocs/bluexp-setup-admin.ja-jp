...
AzureアカウントまたはAzure ADサービスプリンシパルに割り当てることができるAzureカスタムロールを作成します。BlueXPはAzureで認証し、これらの権限を使用してコネクタインスタンスを作成します。
...
.このタスクについて
BlueXPからコネクタを作成すると、デフォルトの設定を使用してAzureに仮想マシンが導入されます。 link:reference-connector-default-config.html["コネクタのデフォルト設定について説明します"]。
