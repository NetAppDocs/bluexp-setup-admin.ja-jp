...
|===
--
コネクタのアウトバウンドネットワーク:: Cloud Managerにログインしたら、Cloud Managerアカウント管理者がクラウドプロバイダまたはオンプレミスネットワークに_ Connector _を導入する必要があります。Connector を使用すると、 Cloud Manager でパブリッククラウド環境内のリソースとプロセスを管理できます。Cloud Managerのすべてのサービスと機能ではなく、ほとんどの場合はコネクタが必要です。 link:concept-connectors.html["コネクターの詳細とその仕組みについては、こちらをご覧ください"]。
+
--
