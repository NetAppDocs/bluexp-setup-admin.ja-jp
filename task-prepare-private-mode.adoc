...
summary: 環境を準備できるように、BlueXPをプライベートモードで運用するために必要なことを理解しておく必要があります。 
---
= プライベートモードでの導入を準備します
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
環境を準備するために、プライベートモードでBlueXPの使用を開始するために必要なことを理解しておく必要があります。
...
HTTP（80）およびHTTPS（443）は、BlueXPコンソールへのアクセスを提供します。SSH （ 22 ）は、トラブルシューティングのためにホストに接続する必要がある場合にのみ必要です。一方、アウトバウンドのインターネット接続が利用できないサブネットにCloud Volumes ONTAP システムを導入する場合は、ポート3128を介したインバウンド接続が必要です。
