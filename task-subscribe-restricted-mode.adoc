...
summary: クラウド プロバイダーのマーケットプレイスから NetApp Intelligent Services にサブスクライブして、BlueXP サービスの料金を時間単位または年間契約で支払います。ネットアップからライセンスを購入した場合は、マーケットプレイスのサービスにサブスクライブする必要もあります。ライセンスは常に最初に課金されますが、ライセンス容量を超えた場合やライセンスの有効期限が切れた場合は、時間単位で課金されます。 
---
...
分類はサブスクリプションを通じて有効になりますが、分類の使用には料金はかかりません。

.作業を開始する前に
データ サービスにサブスクライブするには、マーケットプレイス サブスクリプションをコネクタに関連付けられているクラウド資格情報に関連付ける必要があります。「制限モードで開始する」ワークフローに従っている場合は、コネクタがすでに用意されている必要があります。詳細については、を参照してlink:task-quick-start-restricted-mode.html["制限モードのBlueXPのクイックスタート"]ください。
