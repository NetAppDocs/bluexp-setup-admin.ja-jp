...
+
https://["AzureでLinux VMに接続する方法について説明します"^]
...
+
管理対象の ID を使用すると、 Connector 仮想マシンはクレデンシャルを指定せずに自身を Azure Active Directory に識別できるため、この設定は重要です。完了したら、作成したカスタムロールを割り当てる必要があります https://["Azure リソース用の管理対象 ID の詳細については、こちらをご覧ください"^]。
