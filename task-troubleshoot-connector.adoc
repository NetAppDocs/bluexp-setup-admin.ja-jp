...
NOTE: BlueXP  からAutoSupportメッセージを送信するように選択した場合、BlueXP  はAutoSupportメッセージの送信時に自動的にロードバランシングを行うため、最大5時間かかることがあります。NetAppサポートとの通信に時間がかかる場合は、ファイルをダウンロードしてNetAppサポートに自分で送信してください。
