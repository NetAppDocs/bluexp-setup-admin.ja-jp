...
Cloud Volumes ONTAP システムでAutoSupport メッセージを送信するためのアウトバウンドインターネット接続がない場合、Cloud Managerは、コネクタに含まれているプロキシサーバを使用するようにそれらのCloud Volumes ONTAP システムを自動的に設定します。 link:reference-networking-cloud-manager.html#proxy-server-for-autosupport-messages["プロキシサーバを使用したAutoSupport メッセージの送信の詳細については、こちらをご覧ください"]。
