...
タイムラインにユーザーのログインイベントが表示されるようになりました。これにより、ユーザーがBlueXPにログインした日時を確認できるようになり、監査やセキュリティ監視に役立ちます。組織管理者ロールを持つAPIユーザーは、ログインしたユーザーのメールアドレスをタイムラインに表示できます。  `includeUserData=true``パラメータは次のようになります。  `/audit/<account_id>?includeUserData=true` 。
