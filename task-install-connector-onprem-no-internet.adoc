...
インターネットのないクラウドリージョンでは、BlueXPを使用してCloud Volumes ONTAP システムを導入し、オンプレミスのONTAP クラスタを検出できます（クラウド環境からオンプレミス環境への接続がある場合）。Cloud Backupを使用して、AWSとAzureの商用リージョンのCloud Volumes ONTAP ボリュームをバックアップすることもできます（現時点では、AWSとAzureのシークレットリージョンではバックアップはサポートされていません）。このタイプの環境では、Digital Walletを除き、他のBlueXPサービスはサポートされません。
