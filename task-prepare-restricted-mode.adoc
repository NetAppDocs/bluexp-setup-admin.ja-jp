...
IMDSv1:: EC2インスタンスには、Instance Metadata Service Version 1（IMDSv1）が必要です。このコネクタはIMDSv2ではサポートされていません。
+
--
新しいEC2インスタンスのデフォルトバージョンはIMDSv2になったため、IMDSv1を使用できるようにするには、インスタンスメタデータオプションを変更する必要があります。

https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/configuring-IMDS-existing-instances.html#modify-restore-IMDSv1["AWSのドキュメント：Restore the use of IMDSv1"^]
