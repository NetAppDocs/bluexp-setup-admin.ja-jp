...
** *ネットワーク*：インスタンスにVPC、サブネット、キーペアを指定し、パブリックIPアドレスを有効にするかどうかを選択します。必要に応じてプロキシ設定も指定できます（HTTPとHTTPSがサポートされます）。
+
...
.完了後
Connectorを作成したのと同じAWSアカウントにAmazon S3バケットがある場合、Amazon S3の作業環境がCanvasに自動的に表示されます。 https://docs.netapp.com/us-en/bluexp-s3-storage/index.html["この作業環境でできることの詳細については、こちらをご覧ください"]。
