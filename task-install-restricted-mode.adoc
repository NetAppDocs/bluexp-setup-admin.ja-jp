...
.手順
. にアクセスします https://["AWS MarketplaceのBlueXPページ"^]
...
+
** https://["Azure Marketplaceの一般企業向けページ"^]
** https://["Azure GovernmentリージョンのAzure Marketplaceのページ"^]
...
+
管理対象の ID を使用すると、 Connector 仮想マシンはクレデンシャルを指定せずに自身を Azure Active Directory に識別できるため、この設定は重要です。 https://["Azure リソース用の管理対象 ID の詳細については、こちらをご覧ください"^]。
...
. からConnectorソフトウェアをダウンロードします https://["ネットアップサポートサイト"^]をクリックし、 Linux ホストにコピーします。
+
